 ...
 * param_mode 6: 7 SA parameters
 * param_mode 7: chi, sigma, kappa, cw3
 * param_mode 8: chi, kappa, cw3, cw2
 */
...
#endif
public:
  const unsigned int param_mode = 8;
  NSUtils* nsutils_;
...
public:
  ...
  void init_default_parameter_bound()
  {
    ...
    arma::Row<double> cv1_bnd = {{6.9, 7.3}};
    arma::Row<double> cw3_bnd = {{1.75, 2.5}};
    arma::Row<double> cw2_bnd = {{0.055, 0.3525}};
    arma::Row<double> cb1_bnd = {{0.12893, 0.13700}};
    arma::Row<double> cb2_bnd = {{0.60983, 0.68750}};
    ...
    case 6:
      ...
      mu_bnd_.row(2) = cw3_bnd;
      mu_bnd_.row(3) = cw2_bnd;
      mu_bnd_.row(4) = cv1_bnd;
      mu_bnd_.row(5) = cb1_bnd;
      mu_bnd_.row(6) = cb2_bnd;
      break;
    case 7:
      mu_bnd_.row(0) = fs_chi_log_bnd;
      mu_bnd_.row(1) = sig_bnd;
      mu_bnd_.row(2) = kap_bnd;
      mu_bnd_.row(3) = cw3_bnd;
      break;
    case 8:
      mu_bnd_.row(0) = fs_chi_log_bnd;
      mu_bnd_.row(1) = kap_bnd;
...
public:
  ...
  void load_mda() {
    ...
    double fs_chi = 10.0;  // freestream chi for SA equations
    double sa_scale = 1000;  // SA scale

    eqn.alpha_0_ = alpha;
    eqn.M_0_ = M;
    eqn.Re_0_ = Re;
    eqn.fs_chi_0_ = fs_chi;
  ...
  void setup_eqp() {
    ...
    dg_eqp_c.set_n_max_reduced_basis(15);
    dg_eqp_c.set_weak_greedy_tolerance(3e-5);
    ...
    //dg_eqp_c.generate_structured_parameter_set(5,Xi_train);
    dg_eqp_c.generate_random_parameter_set(50, Xi_train);
  ...
  void set_rb_solver() {
    ...
    ptc_rb.set_max_iter(250);
    ptc_rb.set_initial_timestep(1e0);
    ptc_rb.set_verbosity(-1); 
    ptc_rb.set_abs_tol(5e-8);  
    ptc_rb.set_timestep_increase_multiplier(2.0);
    ptc_rb.set_line_search_type(PTCSolver<double>::LineSearchType::unsteady);
    ptc_rb.set_line_search_maximum_physical_change(0.2); 
    ptc_rb.set_line_search_maximum_residual_increase(1.05);
  ...
  void set_fe_solver() {
    ...
    ptc.set_timestep_increase_multiplier(2.0);
    ptc.set_abs_tol(5e-8);
    ...
    ptc.set_linear_solver_decrease_multiplier(1e-4);

    // set adjoint solver
    adjoint_solver.set_linear_solver(&gmres);
    adjoint_solver.set_residual_tolerance(5e-8);
  };
  ...
  int main(int argc, char *argv[])
  {
    ...
    if (true) {
      ...
      arma::arma_rng::set_seed(0);
      dg_eqp_c.generate_random_parameter_set(n_mc, Xi_mc);  
      arma::Col<double> out_mc(n_mc);

      std::ofstream mf;
      mf.open("out_mc.txt");
  
      eqpd.ptc_rb.set_discretization(&eqpd.dg_eqp_e);
      BlockVector<double>* alpha_rb = eqpd.dg_eqp_e.allocate_vector();
      eqpd.dg_eqp_e.fill_vector(eqpd.eqn.initial_state(), alpha_rb);

      for (unsigned int i_mc = 0; i_mc < n_mc; ++i_mc) {
	eqpd.eqn.set_parameters(Xi_mc.col(i_mc));

	// rb solve
	eqpd.ptc_rb.reset_iteration_count();
	eqpd.ptc_rb.solve(alpha_rb);    
	double j_rb;
	eqpd.dg_eqp_e.compute_output(0, alpha_rb, j_rb);
	out_mc(i_mc) = j_rb;

	for (unsigned int i = 0; i < Xi_mc.n_rows; ++i) {
	  mf << std::setprecision(15) << Xi_mc(i,i_mc) << " ";
	}
